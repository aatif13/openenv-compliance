...
Speed: GROQ_JUDGE_SPEED=fast (default) uses fewer steps and smaller prompts. Groq
calls default to one-at-a-time (GROQ_BASELINE_SERIALIZE=1) and tasks default to
sequential (GROQ_BASELINE_PARALLEL=0) to stay under free-tier TPM; set
GROQ_BASELINE_PARALLEL=1 to overlap tasks (may 429 on low TPM). Use
GROQ_JUDGE_SPEED=default for the older, deeper (slower) baseline.
"""
...
def _is_judge_speed_fast() -> bool:
    """Faster, shallower baselines for demos / judging. Opt out with GROQ_JUDGE_SPEED=default."""
    v = os.environ.get("GROQ_JUDGE_SPEED", "fast").strip().lower()
    return v not in (
        "0", "off", "no", "false", "default", "full", "legacy", "slow", "deep", "thorough",
    )
...
def _groq_baseline_serialize_requests() -> bool:
    """If true, only one Groq request runs at a time (reduces 429s on on_demand TPM)."""
    if _is_judge_speed_fast():
...
def _groq_baseline_max_retries() -> int:
    raw = os.environ.get("GROQ_BASELINE_MAX_RETRIES", "8" if not _is_judge_speed_fast() else "5").strip()
    try:
        n = max(1, min(20, int(raw)))
        if _is_judge_speed_fast():
            return min(n, 6)
        return n
    except ValueError:
        return 5 if _is_judge_speed_fast() else 8
    ...
    *,
    ...
    for attempt in range(max_retries):
        try:
            ...
            if wait is None:
                if _is_judge_speed_fast():
                    wait = min(30.0, 0.45 * (2**attempt))
                else:
                    wait = min(45.0, 0.8 * (2**attempt))
            cap = 50.0 if _is_judge_speed_fast() else 90.0
            wait = min(cap, wait + random.uniform(0.05, 0.35))
            logger.warning(
...
def _active_max_tokens() -> int:
    """
    Completion budget for one JSON action. Groq with response_format json_object
    can need >64 tokens; capping at 80 caused json_validate_failed on tasks 2/3.
    """
    default = "160" if _is_judge_speed_fast() else "128"
    raw = os.environ.get("GROQ_MAX_TOKENS", default).strip()
    try:
        v = max(32, min(512, int(raw)))
        if _is_judge_speed_fast():
            return max(128, min(256, v))
        return max(96, min(512, v))
    except ValueError:
        return 160 if _is_judge_speed_fast() else 128


def _groq_http_timeout() -> float:
    """Per-request timeout for Groq (read); avoids hanging try-it-out for minutes."""
    d = "30" if _is_judge_speed_fast() else "45"
    raw = os.environ.get("GROQ_HTTP_TIMEOUT_SEC", d).strip()
    try:
        t = max(5.0, min(180.0, float(raw)))
        if _is_judge_speed_fast():
            return min(t, 40.0)
        return t
    except ValueError:
        return 30.0 if _is_judge_speed_fast() else 45.0
...
def _llm_doc_char_budget(task_id: int) -> int:
    ...
    for key in (
        ...
        try:
            ...
            continue
    if _is_judge_speed_fast():
        return {1: 7_000, 2: 7_500, 3: 5_500}.get(task_id, 6_500)
...
def _history_tail_limit() -> int:
    d = "2" if _is_judge_speed_fast() else "4"
    raw = os.environ.get("GROQ_BASELINE_HISTORY_TAIL", d).strip()
    try:
        m = 8 if _is_judge_speed_fast() else 12
        return max(0, min(m, int(raw)))
    except ValueError:
        return 2 if _is_judge_speed_fast() else 4
...
def _llm_iteration_cap(task_id: int, env_max_steps: int) -> int:
    ...
    if _is_judge_speed_fast():
        # Task 1 needs up to 6 marks + submit = 7 steps in the worst case; cap others similarly
        per_task = {1: 7, 2: 7, 3: 7}  # ~2× fewer LLM round-trips vs legacy 8+14+18
    else:
        per_task = {1: 8, 2: 14, 3: 18}
    return min(env_max_steps, per_task.get(task_id, 14 if not _is_judge_speed_fast() else 7))
...
def _baseline_parallel_llm_tasks_enabled() -> bool:
    if _is_judge_speed_fast():
        # Default off: 3 tasks in parallel spikes TPM (429 on on_demand). Set to 1 to overlap.
...
def run_llm_agent(seeds: Optional[Dict[int, int]] = None) -> Dict[str, Any]:
    ...
    result = {
        ...
        "meta": {
            ...
            "groq_speed": {
                "judge_speed": "fast" if _is_judge_speed_fast() else "default",
