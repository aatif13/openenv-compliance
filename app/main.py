...
from app.models import Action, StepResponse, GraderResponse
...
# Avoid generic "Internal Server Error" in JSON when something slips past route handlers
# (e.g. Pydantic ValidationError, response encoding issues) so the UI can show a message.
async def _validation_error_handler(_request: Request, exc: ValidationError) -> JSONResponse:
    _log.exception("Pydantic ValidationError")
    return JSONResponse(
        status_code=500,
        content={"detail": f"response/data validation error: {exc!s}"},
    )


async def _unhandled_error_handler(_request: Request, exc: Exception) -> Any:
    if isinstance(exc, (RequestValidationError,)):
        return await _request_validation_exception_handler(_request, exc)  # type: ignore[misc]
    if isinstance(exc, (HTTPException, StarletteHTTPException)):
        return await _http_exception_handler(_request, exc)  # type: ignore[misc]
...
async def custom_docs():
    with open("static/swagger-ui-custom.html", "r", encoding="utf-8") as f:
...
def baseline(
    ...
    for _k, row in (result.get("details") or {}).items():
        if isinstance(row, dict) and "final_score" in row and row["final_score"] is not None:
            row["final_score"] = float(row["final_score"])
        if isinstance(row, dict) and "cumulative_reward" in row and row["cumulative_reward"] is not None:
...
def baseline_llm(
    ...
    **Groq LLM agent** — uses `GROQ_MODEL` (default: fast 8B) to audit documents step-by-step.
    ...
    For faster runs, `GROQ_JUDGE_SPEED` defaults to `fast` (shorter per-task caps, higher
    completion `max_tokens` for valid JSON, serialized Groq calls to reduce 429s, tasks
    sequential by default). Set `GROQ_BASELINE_PARALLEL=1` to run tasks in parallel, or
    `GROQ_JUDGE_SPEED=default` for the older deeper baseline.
    """
    ...
    try:
        ...
        _log.exception("POST /baseline/llm failed")
